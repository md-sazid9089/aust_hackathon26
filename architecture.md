...
**Version:** 1.1 — 2026-09-06 (v1.0 + ADR-13 merge audit + ADR-14–18 AI layer research)
...
| AI orchestration      | LangGraph ≥ 1.2 state graphs; `langchain-openrouter` `ChatOpenRouter` for `openrouter`, `langchain-openai` `ChatOpenAI` for `openai`/`freellmpool`; `OpenAIEmbeddings(base_url=…)` for embeddings | LangChain docs warn `ChatOpenAI`+OpenRouter base_url drops non-standard fields (`reasoning`, provider routing); factory behind one `structured_call` (§28.1). |
...
| `app/ai/budget.py`                      | Deadline       | `RunDeadline(total_s)`: `remaining()`, `call_timeout()=min(LLM_CALL_TIMEOUT_S, remaining−2)`, `may_retry()=remaining≥8`, `skip_optional()=remaining<budget_of_stage`                                                                                                                                                                                                                                                                                                                                                                                                       | settings                                              | class                         | NF-003, AI-002                 |
| `app/ai/schema_lint.py`                 | Guard          | Import-time assert over every output schema: root object, all fields required, `additionalProperties:false`, no defaults, no `$ref` (inlined), depth ≤ 5, ≤ 100 properties, ≤ 1000 enum values                                                                                                                                                                                                                                                                                                                                                                             | pydantic                                              | `lint_all()`                  | AI-001                         |
...
| Timeout           | 10 s                                                                                  | 30 s statement                      | 30 s                                                                    | chat `min(12 s, run remaining − 2)`; embeddings 10 s; extraction chunk 20 s (own 45 s budget)                                         |
...
OpenRouter request shape (BE `ai/client.py`, only when `LLM_PROVIDER=openrouter`, via `ChatOpenRouter`): `openrouter_provider={"require_parameters": True, "sort": "latency", "data_collection": "deny"}`, `models=settings.LLM_FALLBACK_MODELS` (server-side failover on any upstream error — **not** on client timeout), `reasoning={"effort": "none"}` for OpenAI models / `{"effort": "minimal"}` for Gemini; headers `HTTP-Referer`, `X-Title` optional. For `LLM_PROVIDER=openai` and `freellmpool`, `ChatOpenAI` is used and none of these fields are sent — OpenAI rejects unknown body parameters with 400. No `seed` anywhere (temperature 0 suffices; `seed` can exclude endpoints under `require_parameters`). Provider-specific defaults (`LLM_BASE_URL`, model-id prefix) are resolved in `core/config.py` from `LLM_PROVIDER` when the explicit env var is empty.
...
chat(model) -> BaseChatModel
  openrouter : ChatOpenRouter(model, temperature=0, max_retries=0, openrouter_provider={...}, reasoning={effort}, models=LLM_FALLBACK_MODELS)
  openai     : ChatOpenAI(base_url, api_key, model, temperature=0, max_retries=0)
  freellmpool: ChatOpenAI(base_url=127.0.0.1:8080/v1, model="auto")
  mock       : MockChat(fixtures by purpose)        # only if ENV=demo and course.is_demo, or under pytest
...
Every analysis run gets `RunDeadline(LLM_RUN_DEADLINE_S=27)`; NF-003 hard limit is 30 s. Per-stage budgets (p50 targets, exam_audit, ≤ 40 questions):

| Stage                                                       | Parallelism                                                                       | Budget     | Optional?                            |
| ----------------------------------------------------------- | --------------------------------------------------------------------------------- | ---------- | ------------------------------------ |
| load + stats from confirmed map                             | —                                                                                 | 0.5 s      | no                                   |
| embed_questions (skip rows with matching `embedding_model`) | 1 batch ≤ 64                                                                      | 1.5 s      | no                                   |
...
| confirm_duplicates                                          | ≤ 4 calls, `Semaphore(4)`, 10 pairs/call, ≤ 40 pairs, `max_completion_tokens` 600 | 6 s        | yes                                  |
| build_findings + persist                                    | —                                                                                 | 0.7 s      | no                                   |
| **Sum**                                                     |                                                                                   | **≈ 17 s** | 10 s headroom for one fallback/retry |
...
### 28.4 Graphs (LangGraph ≥ 1.2 `StateGraph`, state = TypedDict; fan-out = `asyncio.gather` + `Semaphore(4)` inside a node, no `Send`)
...
### 28.5 Output schemas (Pydantic, all `extra='forbid'`, all fields required, depth ≤ 5, no `$ref`)
...
- `evidence_quote` must be a verbatim substring of the wrapped document after NFC normalisation, whitespace collapse, soft-hyphen/ZWJ stripping; fallback `rapidfuzz.partial_ratio ≥ 90`; otherwise `findings.evidence_snippet` = the question/topic text itself with `provenance.evidence_source='source_text'` (keeps AI-003 non-null).
...
2. Where `refusal` and `finish_reason` surface in `ChatOpenRouter` `include_raw` output (`raw.response_metadata` vs `additional_kwargs`).
3. Measured p95 of a 6-question MAP_AND_BLOOM call on `gpt-5.6-luna` ≤ 10 s (else reduce batch to 4).
...
Tasks: DB-01 `0001–0002` extensions, role, enums → DB-02 `0003` profiles + trigger → DB-03 `0004` courses/outcomes/topics + PO seed → DB-04 `0005–0006` artefacts, questions, maps, marks, rubric, answers, grader_scores → DB-05 `0007–0008` runs, events, findings, attainment_results, prescores, usage_logs, `0013` llm_cache → DB-06 `0009` indexes (HNSW) → DB-07 `0010` RLS policies → DB-08 `0011` functions (`similar_questions`, `compute_co_attainment`, `reset_demo`, `seed_demo`) → DB-09 `0012` views → DB-10 seed fixtures (author demo documents + CSV) → DB-11 SQL tests + `scripts/*` → DB-12 CI migration job (with BE) → DB-13 query review of BE repositories (EXPLAIN on hot paths) → DB-14 backup/restore drill on Supabase project.
...
| ADR-05 | OpenAI-compatible gateway abstraction: `LLM_PROVIDER` ∈ {`openai`, `openrouter`, `freellmpool`, `mock`}; OpenAI direct used with the team key, OpenRouter as alternate, freellmpool dev-only                    | vendor SDKs per provider; OpenRouter-only                                     | one adapter; free dev path; env-only switch; two paid providers = outage hedge                                                                                                                                                                                                                                             |
...
| ADR-15 | Adapter = `ChatOpenRouter` (openrouter) / `ChatOpenAI` (openai, freellmpool) behind one `structured_call`; `include_raw=True`; no `seed`                                                                        | `ChatOpenAI` with OpenRouter `base_url` for all                               | LangChain documents that `ChatOpenAI` drops OpenRouter-specific fields (`reasoning`, `provider`); `seed` + `require_parameters` can leave zero eligible Gemini endpoints                                                                                                                                                   |
